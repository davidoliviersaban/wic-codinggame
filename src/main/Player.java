...
class Player {
  ...
  public static final int     MAX_CHECKPOINTS     = 10;
  ...
  public static class Unit extends Point {
    ...
    public String toString() {
      return String.format("{id:%d, position:%s, vx:%.2f, vy:%.2f, radius:%d}", id, super.toString(), vx, vy,
          (int) radius);
    }
  ...
  public static class Pod extends Unit {
    ...
    public String toString() {
      return String.format("{Unit: %s, angle: %d, thrust: %d, boost: %d, targetId: %d}", super.toString(), (int) angle,
          (int) thrust, boost, nextCheckpointId);
    }
  ...
  public static class Move {
    ...
    public String toStringOutput() {
      if (thrust > 100)
        return String.format("%d %d BOOST", (int) Math.round(target.x), (int) Math.round(target.y));
      ...
      while (t < 1.0) {
        ...
        for (int i = 0; i < pods.length; ++i) {
          ...
          for (int j = i + 1; j < pods.length; ++j) {
            ...
            // alors on la garde
            if (ACTIVATE_COLLISIONS && col != null && col.t + t < 1.0
  ...
  public static class IAWood {
    public static Move play(Game game, int podId) {
      ...
      return new Move(diffAngle, pod.thrust, target);
    }
  }
  ...
  public static class Game {
    ...
    public Game clone() {
      ...
      int i = 0;
      for (Pod pod : pods)
        if (pod != null) game.pods[i++] = pod.clone();
    ...
    public void initGame(Checkpoint cp, Pod[] pods) {
      ...
      addCheckpoint(new Checkpoint(0, baryX / pods.length, baryY / pods.length));
      addCheckpoint(cp);
    ...
    public Checkpoint addCheckpoint(Checkpoint cp) {
      for (int i = 0; i < checkpoints.length; i++) {
        ...
        else if (checkpoints[i].equals(cp))
          return checkpoints[i];
      }
      return null;
    }
  ...
  private static Game initRound(Scanner in, Game g, boolean firstRound) {
    ...
    game.nextCheckpointAngle = in.nextInt(); // angle between your pod
    ...
    if (firstRound) {
      ...
      nextCheckpoint = game.addCheckpoint(nextCheckpoint);
      //game.pods[0].vx = x - game.pods[0].x;
      //game.pods[0].vy = y - game.pods[0].y;
      game.pods[0].x = x;
      game.pods[0].y = y;
      game.pods[0].nextCheckpointId = nextCheckpoint.id;

      //game.pods[1].vx = opponentX - game.pods[1].x;
      //game.pods[1].vy = opponentY - game.pods[1].y;
      game.pods[1].x = opponentX;
      game.pods[1].y = opponentY;
    }
    return game;
  }
  ...
  public static void main(String args[]) {
    ...
    // game loop
    while (true) {
      game = initRound(in, game, firstRound);
      firstRound = false;
      System.out.println(IABronze.play(game, 0).toStringOutput());
    }
  }
